...
public class SalesItemDAOIMPL implements SalesItemDAO {
    ...
    public void saveSalesItem(SalesItem salesItem) {
        ...
        try (Connection conn = new Connect().connectToDB(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public boolean decreaseItem(int quantity, int salesItemId) {
        ...
        try (
                Connection conn = new Connect().connectToDB(); PreparedStatement prs = conn.prepareStatement(sql);) {
    ...
    public List<SalesItem> getAllSalesItems() {
        ...
        List<SalesItem> salesItems = new ArrayList<>();
        String sql = "SELECT sales_item_ID, sales_ID, product_ID, quantity, unit_price FROM sales_items";
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql); ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                ...
                salesItem.setSales_item_ID(resultSet.getInt("sales_item_ID"));
                salesItem.setSales_ID(resultSet.getInt("sales_ID"));
                salesItem.setProduct_ID(resultSet.getInt("product_ID"));
                salesItem.setQuantity(resultSet.getInt("quantity"));
                salesItem.setUnit_price(resultSet.getBigDecimal("unit_price"));
